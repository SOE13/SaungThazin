...
const UserEditScreen = ({navigation, route}) => {
  ...
  const [imageReselect, setImageReselect] = useState(false);
    ...
    if (imageReselect) {
      ...
      let uri = reference.getDownloadURL().then(downloadURL => {
        firestore()
          .collection('user')
          .doc(id)
          .update({amount, imageUri: downloadURL})
          .then(() => {
            setLoading(true);
            let record = amount - tempAmount;
            updateAmountRecord(name, account, accountNo, downloadURL, record);
            setLoading(true);
            navigation.goBack();
